...
w)
  - a,b,c,d 分別是列聯表內的四個數值
  - n 是總樣本數
  - $\binom{x}{y}$ 表示組合數（即 "x 選 y"）

Fisher’s Exact Test 不適用於大規模數據，因為超幾何計算會變得過於複雜。

#### 4. Fisher’s Exact Test vs. 卡方檢定（Chi-Square Test）
|                 | Fisher’s Exact Test （費雪精確檢定）| Chi-Square Test (卡方檢定) |
|---------------|-------------------|----------------|
| **適用場景**  | 小樣本，尤其是某些格子頻數 < 5 | 大樣本，格子頻數較高 |
| **計算方式**  | 基於超幾何分佈，計算精確 P 值 | 近似卡方分佈，計算期望值與觀察值差異 |
| **P 值精確性** | 更準確 | 近似方法，可能有誤差 |
| **運算速度** | 速度較慢，尤其是大數據時 | 速度較快，適合大規模數據 |

選擇建議
- 若 樣本量大（所有格子的期望頻數 ≥ 5）➡ 卡方檢定
- 若 樣本量小或部分格子數量過少（任一格子 < 5）➡ Fisher’s Exact Test
在微生物學、生物資訊學和臨床研究中，當樣本量較小時，通常 Fisher’s Exact Test 比較適合。

#### 5. 應用場景
Fisher’s Exact Test 廣泛應用於生物醫學、流行病學和微生物學：
- 藥物試驗：比較治療組與對照組的效果（例如：某藥物是否顯著減少疾病發生）。
- 臨床研究：檢測某種基因突變是否與某種疾病相關。
- 微生物研究：分析 特定細菌 在不同樣本中的分佈是否顯著不同。
在 QIIME2 微生物組學分析中，如果我們想比較不同群組（例如 用藥 vs. 無用藥）的細菌分類豐度，當某些細菌的豐度很低時，通常會使用 Fisher’s Exact Test 而不是 卡方檢定。
